...
def does_db_exist(db: plyvel.DB, server_id: bytes or int):
  """
  ...
  if type(server_id) != bytes:
    server_id = bytes(
      str(server_id)[:7],
      encoding='utf8')  # Grab first 8 chars from ID since a byte can't
    # exceed 8 bits
  if db.get(server_id):
    return True
  else:
...
class DBHelper:
  ...
  def __init__(self, server_id: int, db: plyvel.DB):
    """
        ...
        """
    self.server_id = bytes(
      str(server_id)[:7], encoding='utf8'
    )  # Grab first 8 chars from ID since a byte can't exceed 8 bits
    self.db = db
