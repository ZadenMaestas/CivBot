...
def main():
    ...
    statuses = cycle(["Use /help for help", "Made By ZadenMaestas and LemonJuice"])
    ...
    if not TESTING:
        @bot.event
        async def on_slash_command_error(interaction: discord.ApplicationCommandInteraction,
                                         error: commands.CommandError):
            """
            Called when an error occurs in a slash command
            """
            if isinstance(error, commands.CommandOnCooldown):
                embed = discord.Embed(colour=discord.Colour.red(), title="An Error Has Occurred:",
                                      description=f"This command is on cooldown, you can use it in `{round(error.retry_after, 2)} seconds`")
            else:
                embed = discord.Embed(colour=discord.Colour.red(), title="An Error Has Occurred:", description=error)
            await interaction.send(embed=embed)
    elif TESTING:
        @bot.slash_command()
        async def log_db(inter: discord.ApplicationCommandInteraction):
            print(DBHelper(inter.guild_id, db).fetch_server_data())
            embed = discord.Embed(colour=EMBED_COLOUR, title="Server DB has been logged to terminal.")
    ...
    async def on_disconnect():
        db.close() and print('Database was successfully closed') if not db.closed else print(
    ...
    async def init(inter: discord.ApplicationCommandInteraction):
        if not does_db_exist(db, inter.guild_id):
            embed = discord.Embed(colour=discord.Colour.green(), title="Bot Config Has Initialized Successfully")
            DBHelper(inter.guild_id, db)
        else:
            embed = discord.Embed(colour=discord.Colour.red(), title="Bot Has Already Been Initialized")
        await inter.send(embed=embed)

    @bot.slash_command()
    async def create_empire(inter: discord.ApplicationCommandInteraction, empire_name: str):
        """
        ...
        embed = discord.Embed(colour=EMBED_COLOUR, title=f"A new empire `{empire_name}` has been created!",
    ...
    async def game_map(inter: discord.ApplicationCommandInteraction):
        ...
        await inter.send(Empire(empire_data[0], inter.author.id, inter.guild_id, db).load_empire_map())
